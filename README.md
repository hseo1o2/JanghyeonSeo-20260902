...
- 상대 장면에 `ㅋㅋ` / `오` / `궁금` / `♥`
- **지금 올리기** (출근·점심·운동·밤)
- 방금 본 장면으로 말 걸기 (ChatGPT가 첫 질문만 제안)

양방향 실사용자 매칭과 실시간 채팅은 넣지 않았습니다. 상대의 짧은 반응은 로그 맥락에 맞춰 시뮬됩니다.
...
ChatGPT는 발견 카드에 나오지 않습니다. `candidate_interested` 이후, 로그에서 장면을 보고 말을 걸 때만 첫 질문 3개를 제안합니다. AI를 먼저 보여주면 "AI가 흥미로워서 클릭한 것"과 "일상이 흥미로워서 클릭한 것"을 나눌 수 없습니다.
...
실제 상대가 오늘 장면을 올리는 양방향 로그는 아닙니다. 내가 올린 뒤에 상대의 짧은 리액션·답장이 따라오게 해서, 셋로그식 경험이 끊기지 않게만 했습니다. 다음 단계라면 두 사람이 같은 방에 들어와 직접 장면을 올리는 쪽이 맞습니다.
